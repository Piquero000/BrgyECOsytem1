...
function viewTaguig() {
  ...
  const infoBanner = `
    <div class="max-w-7xl mx-auto p-4 mb-4 bg-yellow-50 rounded-lg border">
      <div class="text-sm text-yellow-700 font-semibold">Public Complaint History</div>
      <div class="text-xs text-yellow-800 mt-1">This list shows the public history of complaints. Complaints removed from the admin list remain visible here so residents can still view past reports.</div>
    </div>
  `;
  if (sorted.length===0) {
    return `
  ...
  return `
    <div class="p-6">
      ...
      <div class="max-w-7xl mx-auto grid gap-6 md:grid-cols-2">
        ${sorted.map(c => `
          <div class="bg-white p-5 rounded-xl shadow complaint-card">
            <div class="flex justify-between items-start mb-2">
              <div>
                <h3 class="font-bold text-lg">${c.title}</h3>
                ${c.admin_removed ? `<div class="text-xs text-gray-500 mt-1">Archived: removed from admin list</div>` : ''}
              </div>
    ...
    <div class="mb-4">
      <table class="w-full text-sm">
        ...
        <tr><td class="font-semibold w-36">Phone</td><td>${phone || '<span class="text-gray-400">(not set)</span>'}</td></tr>
          <tr><td class="font-semibold w-36">Street Location</td><td>${barangay || '<span class="text-gray-400">(not set)</span>'}</td></tr>
          ...
          <div id="verifySection">
            ...
            ${verifiedStatus === 'Verified' ? `
              <div class="mb-3 p-3 bg-green-50 rounded">
                <p class="text-sm">Your account is verified and cannot be changed or removed from this panel. If you need to update verification details, please contact an administrator.</p>
              </div>
            ` : `
